<template>
  <div style="display: block;padding-left: 30px;padding-right: 30px;padding-top: 20px" v-if="isshow">
      ...
      <div style="margin-top: 15px;width: 100%;" class="display-row" v-if="notitype === 3">
          <div class="filelabel row-left" style="padding-left: 30px"><p>时间范围:</p></div>
          <div class="titleinput row-right" style="padding-left: 10px">
              <p>{{getdate(timeRange[0])}} - {{getdate(timeRange[1])}}</p>
          </div>
      </div>
    ...
    methods:{
        ...
        setData(data){
            ...
            if (data.surveyRange){
                ...
                if (data.surveyRange.length > 3) {
                    this.notifyRange = data.surveyRange[0].userInfo.name + ' ' + data.surveyRange[1].userInfo.name + ' ' + data.surveyRange[2].userInfo.name + ' ';
                } else {
