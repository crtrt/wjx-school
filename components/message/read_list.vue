...
<!--            <el-checkbox v-model="fieldShow.showTiJiaoTime">提交时间</el-checkbox>-->
        ...
        <div  style="display: table; width: 100%; padding-top: 20px; padding-bottom:15px;">
            <div style="display: table-row">
                ...
                <div v-show="fieldShow.showTiJiaoTime" class="tableTitle" style="min-width: 150px">
                    提交时间
                </div>
...
<!--                </div>-->
            ...
            (index % 2 === 0)}" style="display: table-row;" v-if="dataList.length > 0" v-for="(item,index) in dataList" :key="item.id">
                ...
                <div v-show="fieldShow.showTiJiaoTime" class="tableCell" style="min-width: 40px">
                    {{item.update_time}}
                </div>
        ...
        methods:{
            ...
            getFinishedList(isFlesh) {
                ...
                API.adminReadList(data).then(res => {
                    ...
                    this.noHaveAttachment = res.is_empty_attachment;
            ...
            downloadBatch () {
                ...
                API.downloadZhengji(data).then(res => {
                    if(res.code){
                        alert(res.message);
                        return;
                    }
                    alert('添加下载任务成功，由于附件空间较大，请于10分钟后于下载任务查看');
                }).catch(msg => {
                    alert('操作失败')
                })
            ...
            downloadEnclosure(index){
                this.attachment=this.dataList[index].attachment;
                this.download=true;
            },
